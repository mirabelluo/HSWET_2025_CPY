// Updated 12/10/2024
// Turbine sweep picthes for test on 12/11/2024
...
void setup() {
  ...
  delay(pitchPositionHoldTime); // [!] New
  //linActNewPos(1100);
...
void loop() {
  ...
  if (millis() - previousTime > dataLoggingInterval) {
    ...
    SensorData data;
    data.voltage = ina260.readBusVoltage() / 1000.0; // Convert from mV to V
    data.current = ina260.readCurrent() / 1000.0; // Convert from mA to A
    data.rpm = rpm;
