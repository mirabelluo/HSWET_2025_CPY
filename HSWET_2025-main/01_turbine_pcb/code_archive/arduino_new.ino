...
// encoder variables
// 12.10 NEW: actuator pin is now 38
int ACTUATOR_PIN = 6; // White
int ENCODER_PIN = 2;  // Orange
...
int positions[] = {1200, 1250, 1300, 1350, 1400, 1450, 1480};
...
void setup() {
  ...
  Wire.begin(4, 5); // SDA on GPIO 8, SCL on GPIO 10
  ...
  Serial.println("Actuator is on actuator pin");
...
void loop() {
  ...
  if (millis() - previousSettingTime > settingLoggingInterval) {
    ...
    linActNewPos(positions[i]);
    Serial.println("waiting in between positions...");
    // delay(7000);
    delay(3000);
    previousSettingTime = millis();
  }
...
void updateEncoder() {
  counter++;
  // Serial.println("encoder is being updated");
}
...
void linActNewPos(int interval) {
  ...
  for (int i = 0; i < 100000; i++) {
    if (micros() > timer) {
      ...
      state = !state;
      // Serial.println("timer is going");
      digitalWrite(ACTUATOR_PIN, state);
    }
  }
  // Serial.println("ACTUATOR PIN STATE LOW");
